      ...
      <>
        ...
        {userData.isEditingProfile && (
          <>
            ...
            {userData.updateProfileErrorMessage == "ERROR_MESSAGE" && <p id="error_message" style={{color:"red"}}>Unable to save changes. Please check your information and try again.</p>}
            {userData.updateProfileErrorMessage == "SOMETHING_WENT_WRONG_MESSAGE" && <p id="something_went_wrong_message" style={{color:"red"}}>An error occurred. Please try again.</p>}
            {userData.updateProfileErrorMessage == "PFP_ERROR_MESSAGE" && <p id="pfp_error_message" style={{color:"red"}}>Only PNG files are allowed.</p>}
            {userData.updateProfileErrorMessage == "PFP_TOO_LARGE" && <p id="pfp_too_large" style={{color:"red"}}>Only PNGs up to 16MB are accepted.</p>}
            {userData.updateProfileErrorMessage == "ATTACHMENT_ERROR_MESSAGE" && <p id="attachment_error_message" style={{color:"red"}}>Only PDF files are allowed.</p>}
            {userData.updateProfileErrorMessage == "ATTACHMENT_TOO_LARGE" && <p id="attachment_too_large" style={{color:"red"}}>Only PDFs up to 16MB are accepted.</p>}
            {/*--------------------------*/}
          </>
        )}
        {/*-------------------------------------------------------------*/}
