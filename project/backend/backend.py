...
"""
Function that returns initial profile data when called.
"""
@webApp.route("/p/<user_id>/getData", methods = ["GET"])
@login_required
def getInitialProfileData(user_id):
    ...
    editing_disabled = int(current_user.id) != user_id # Editing is disabled if the userID belonging to the viewing profile is not the current user's ID
    print(f"BACKEND: CURRENT USER ID: {current_user.id} USER ID OF PROFILE: {user_id} IS EDITING DISABLED {editing_disabled}")
    ...
    return jsonify({ # Return all of the needed user data in the form of JSON to the front end
        ...
        "lname": escape(lname),
        "bio": escape(bio),
        "deleteAccLink": url_for("onDeleteAccount", user_id=user_id), # Tell flask to find the absolute link for the function onDeleteAccount() and pass user_id as the URL parameter
        "username": escape(username),
        "isEditingProfile": False,
        "editingDisabled": editing_disabled,
        "profilePicture": None,
